...
#=====attendence========
    ...
    def mark_attendence(self,i,r,n,d):
        with open(r"D:\attendence management software\attendance_report\abdul.csv","r+",newline="\n") as f:
            my_data=f.readlines()
            name_list=[]
            for line in my_data:
                entry=line.split((","))
            ...
            #else: 
                #all(item not in name_list for item in [i,r,n,d])
                #now = datetime.now()
                #next_day = now + timedelta(days=1)  # Calculate the next day's date

                #d1 = next_day.strftime("%d/%m/%Y")
                #dtString = now.strftime("%H:%M:%S")
    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                no,predict=clf.predict(gray_image[y:y+h,x:x+w])
                ...
                try:
                    ...
                    my_cursor.execute("select Name from student where studentID="+str(no))
                    ...
                    my_cursor.execute("select roll from student where studentID="+str(no))
                    ...
                    my_cursor.execute("select dep from student where studentID="+str(no))
                    ...
                    my_cursor.execute("select studentID from student where studentID="+str(no))
